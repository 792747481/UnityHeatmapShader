...
public class QuadScript : MonoBehaviour
{
    ...
    [SerializeField] int N = 10; // 固定点的数量
    ...
    void Init()
    {
        ...
        mPoints = new float[32 * 3]; // 最多 32 点
        mHitCount = Mathf.Min(N, 32); // 不能超过 32

        // 随机生成 N 个点（分布在 [-2,2] 范围）
        for (int i = 0; i < mHitCount; i++)
        {
            mPoints[i * 3] = Random.Range(-2f, 2f); // x
            mPoints[i * 3 + 1] = Random.Range(-2f, 2f); // y
            //mPoints[i * 3 + 2] = Random.Range(1f, 3f);  // 强度
            mPoints[i * 3 + 2] = 1;
        }
